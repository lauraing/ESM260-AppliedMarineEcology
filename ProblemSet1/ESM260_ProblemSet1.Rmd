...
moundingcoral_plot <- ggplot(moundingcoral, aes(x = location, y = density_habitat, fill = location)) +
  ...
  scale_x_discrete(expand = c(0,0)) +
  scale_fill_manual(name = "Lagoon", breaks = c("Aitutaki", "Rarotonga"), values = alpha(c("coral", "goldenrod2"), 0.75)) +
  theme_bw() +
  theme(text = element_text(family = "serif")) +
  theme(plot.title = element_text(size = 12, hjust = 0.5, face = "bold")) +
  theme(panel.background = element_blank(),
...
# Surgeonfish (adult surgeonfish density vs fine-branching coral density)
...
# Coral Trout (adult coral trout density vs coarse-branching coral density)
...
# Spotted Damselfish (adult spotted damselfish density vs sea anemones density)
...
# Yellow Damselfish (adult yellow damselfish density vs mounding coral density)
